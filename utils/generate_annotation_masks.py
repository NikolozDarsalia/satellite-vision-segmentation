...
def process_image(json_file):
    ...
    for feat in js1['features']['xy']:
        ...
        msk[_msk > 0] = 1 #buildig annotation
    ...
    #EN: changed '/labels/' into 'labels' and same for masks. Otherwise it won't save the png into the masks file instead it was
    #saving into the labels folder. 
...
if __name__ == '__main__':
    ...
    dirs = [os.path.join(args.input, f) for f in os.listdir(args.input) if os.path.isdir(os.path.join(args.input, f))]
    print("dirs", dirs)
    all_files = []
    for d in dirs:
        makedirs(path.join(d, masks_dir), exist_ok=True)
        for f in sorted(listdir(path.join(d, 'labels'))):
            if '_pre_disaster.json' in f:
                all_files.append(path.join(d, 'labels', f.replace('_pre_disaster.json', '_pre_disaster.json')))
                #f.replace('_pre_disaster.json', '_pre_disaster.json') could be simply f
    with Pool() as pool:
