...
class Evaluator(object):
    ...
    def F1_score_list(self):
        Rec = np.diag(self.confusion_matrix) / (np.sum(self.confusion_matrix, axis=0) + 1e-7)
        Pre = np.diag(self.confusion_matrix) / (np.sum(self.confusion_matrix, axis=1) + 1e-7)
    ...
    def Damage_F1(self):
        ...
        damage_f1 = (self.num_class)/(1./f1_score_list[0]+1./f1_score_list[1])
        return damage_f1
    ...
    def _generate_matrix(self, gt_image, pre_image):
        gt_image, pre_image = np.asarray(gt_image.cpu()), np.asarray(pre_image.cpu())
        mask = (gt_image >= 0) & (gt_image < self.num_class)
        label = self.num_class * pre_image[mask].astype(np.uint8) + gt_image[mask].astype(np.uint8)
